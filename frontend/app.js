...
async function fetchStats() {
    try {
        ...
        document.getElementById('total-alerts').textContent = stats.total_alerts_24h || 0;
        document.getElementById('critical-alerts').textContent = stats.critical_alerts_24h || 0;
        document.getElementById('suspicious-volume').textContent = formatCurrency(stats.total_suspicious_volume_24h);
        document.getElementById('avg-score').textContent = stats.avg_suspicion_score || 0;
        document.getElementById('clusters-detected').textContent = stats.wallet_clusters_detected || 0;
        document.getElementById('last-scan').textContent = formatTime(stats.last_scan);
    } catch (error) {
...
function renderAlerts() {
    ...
    if (filtered.length === 0) {
        container.innerHTML = `
            <div class="empty-state">
                <div class="empty-state-icon">🔍</div>
                <p>No ${currentFilter === 'all' ? '' : currentFilter} alerts detected yet</p>
    ...
    container.innerHTML = filtered.map(alert => createAlertCard(alert)).join('');
}
...
function createAlertCard(alert) {
    ...
    return `
        <div class="alert-item ${alert.severity}" onclick="showAlertDetail('${alert.id}')">
            <div class="alert-severity">
                <span class="severity-badge ${alert.severity}">${alert.severity}</span>
                <span class="suspicion-score" style="color: ${getSeverityColor(alert.severity)}">${Math.round(alert.suspicion_score)}</span>
            </div>
            <div class="alert-content">
                <div class="alert-header">
                    <div class="alert-market">${truncate(trade.market_question, 80)}</div>
                    <div class="alert-time">${formatTimeAgo(alert.created_at)}</div>
                </div>
                <div class="alert-flags">
                    ${flags.map(flag => `<span class="flag-tag">${flag}</span>`).join('')}
                </div>
                <div class="alert-trade-info">
                    <div class="trade-detail">
                        <span class="label">Side:</span>
                        <span class="value ${trade.side.toLowerCase()}">${trade.side}</span>
                    </div>
                    <div class="trade-detail">
                        <span class="label">Size:</span>
                        <span class="value">${formatCurrency(trade.notional_usd)}</span>
                    </div>
                    <div class="trade-detail">
                    ...
                    <div class="trade-detail">
                        ...
                        <span class="value">${trade.potential_return_pct ? `+${trade.potential_return_pct.toFixed(0)}%` : '--'}</span>
                    </div>
                    <div class="trade-detail">
    ...
    container.innerHTML = markets.map(market => `
        <div class="market-item">
            ...
            <div class="market-stats">
                ...
                <span class="market-stat">
                    <span class="value">${formatCurrency(market.total_suspicious_volume)}</span>
                </span>
    ...
    container.innerHTML = clusters.map(cluster => `
        <div class="cluster-item">
            <div class="cluster-header">
                <span class="cluster-wallets">🕸️ ${cluster.wallet_count} wallets</span>
                <span class="cluster-correlation">${(cluster.correlation_score * 100).toFixed(0)}% correlated</span>
            </div>
            <div class="cluster-stats">
                Volume: ${formatCurrency(cluster.total_volume)} • Detected: ${formatTimeAgo(cluster.first_detected)}
            </div>
        </div>
    `).join('');
}
...
function showAlertDetail(alertId) {
    ...
    document.getElementById('modal-body').innerHTML = `
        <div class="modal-header">
            <div class="modal-title">${trade.market_question}</div>
            <div class="severity-badge ${alert.severity}" style="display: inline-block; margin-top: 8px;">
        ...
        <div class="modal-section">
            ...
            <div class="detail-grid">
                <div class="detail-item">
                    ...
                    <div class="detail-value ${trade.side === 'BUY' ? 'highlight' : 'danger'}">${trade.side}</div>
                </div>
                <div class="detail-item">
                    <div class="detail-label">Notional</div>
                    <div class="detail-value highlight">${formatCurrency(trade.notional_usd)}</div>
                </div>
                <div class="detail-item">
                ...
                <div class="detail-item">
                    ...
                    <div class="detail-value highlight">${trade.potential_return_pct ? `+${trade.potential_return_pct.toFixed(0)}%` : '--'}</div>
                </div>
                <div class="detail-item">
                    <div class="detail-label">Timestamp</div>
                    <div class="detail-value">${formatTime(trade.timestamp)}</div>
                </div>
            </div>
        </div>
            ...
            <div class="detail-grid">
                <div class="detail-item">
                    ...
                    <div class="detail-value" style="font-size: 0.75rem;">
                        <a href="https://polymarket.com/@${wallet.address}" target="_blank" style="color: var(--accent-secondary);">
                ...
                <div class="detail-item">
                    ...
                    <div class="detail-value ${wallet.total_trades < 10 ? 'danger' : ''}">${wallet.total_trades}</div>
                </div>
                <div class="detail-item">
                    <div class="detail-label">Unique Markets</div>
                    <div class="detail-value ${wallet.unique_markets < 5 ? 'danger' : ''}">${wallet.unique_markets}</div>
                </div>
                <div class="detail-item">
                    <div class="detail-label">Lifetime Volume</div>
                    <div class="detail-value">${formatCurrency(wallet.total_volume_usd)}</div>
                </div>
                <div class="detail-item">
                    <div class="detail-label">Win Rate</div>
                    <div class="detail-value ${wallet.win_rate > 0.8 ? 'highlight' : ''}">
                        ${wallet.win_rate ? `${(wallet.win_rate * 100).toFixed(0)}%` : 'N/A'}
                    </div>
                </div>
                <div class="detail-item">
                    <div class="detail-label">Wallet Score</div>
                    <div class="detail-value danger">${wallet.suspicion_score}</div>
                </div>
            </div>
        </div>
        ...
        <div class="modal-section">
            <h3>🚩 Flags</h3>
            <div class="alert-flags">
                ${alert.flags.map(flag => `<span class="flag-tag">${flag}</span>`).join('')}
            </div>
        </div>
        ...
        <div class="modal-section">
            ...
            <div style="display: flex; align-items: center; gap: 16px;">
                <div style="flex: 1; height: 8px; background: var(--bg-tertiary); border-radius: 4px; overflow: hidden;">
                    <div style="width: ${alert.insider_probability * 100}%; height: 100%; background: linear-gradient(90deg, var(--accent-primary), ${alert.insider_probability > 0.7 ? 'var(--severity-critical)' : 'var(--accent-secondary)'});"></div>
                </div>
                <span style="font-family: var(--font-mono); font-size: 1.2rem; font-weight: 700; color: ${getSeverityColor(alert.severity)};">
...
async function triggerScan() {
    ...
    btn.innerHTML = '<span class="spinner" style="width: 16px; height: 16px; border-width: 2px;"></span> Scanning...';
    btn.disabled = true;
